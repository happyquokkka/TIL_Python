...
string.count('m')   # string이라는 변수에 m이 몇개야?
...
# 문자열과 리스트의 차이
for c in colors_split : # 리스트 출력 -> 리스트 내 각 요소별로 대입
    print(c)

for c in colors : # 문자열 출력 -> 문자열 내 각 문자별로 대입
    print(c)
...
# 이 문장 수행 후 course 변수의 값은 변경되는가? ㅡ 원본을 변경시키지는 않는다!
# 변경해서 반환할 뿐!

# Web -> Python으로 변경
print(course.replace('Web','Python'))  # Web이 없으니 원래 문자열 그대로 반환
...
# upper() : 대문자로 / lower() : 소문자로 / capitalize() : 문장의 첫번째 문자를 대문자로
...
name = input('이름 입력: ')
if not (name.isalpha()) :   # 문자만 입력된 게 아니라면
    print('문자만 입력하세요.')
ID = input('ID 입력 : ')
...
# 스페이스 여부 확인 - 스페이스로만! 이루어져 있는지를 확인
print(' '.isspace())  # 스페이스 한 개인 경우
print(' c'.isspace())  # 다른 문자와 혼합된 경우
